...
from src.agents.reinforcement_learning.encoder_policy import PolicyEncoder
from src.agents.reinforcement_learning.encoder_policy import ValueEncoder
from src.agents.reinforcement_learning.encoder_policy import CombinedPolicyEncoder
from src.agents.reinforcement_learning.encoder_policy import CombinedValueEncoder
...
# compatability dictionaries
AGENT_POLICY_DICT: Dict[str, str] = {'self-attention': 'PolicyEncoder', 'mlp': 'PolicyNetwork',
                                     'combi_encoder': 'CombinedPolicyEncoder'}
AGENT_VALUE_DICT: Dict[str, str] = {'self-attention': 'ValueEncoder', 'mlp': 'ValueNetwork',
                                    'combi_encoder': 'CombinedValueEncoder'}
...
class PolicyNetwork(nn.Module):
    ...
    :param obs_dims: Observation size to determine input dimension
    :param action_dim: Number of action to determine output size
    :param learning_rate: Learning rate for the network

    """

    def __init__(self, obs_dim: List, action_dim: int, learning_rate: float, **kwargs):

        super(PolicyNetwork, self).__init__()

        input_dim = obs_dim[0]
        activation = kwargs.get('policy_activation', POLICY_ACTIVATION)
        hidden_layers = kwargs.get('policy_layer', POLICY_LAYER)
        ...
        for i, layer_dim in enumerate(hidden_layers):
            if not i + 1 == len(hidden_layers):
                ...
                # create output layer
                net_structure.extend([nn.Linear(layer_dim, action_dim), nn.Softmax(dim=-1)])
...
class ValueNetwork(nn.Module):
    ...
    def __init__(self, obs_dim: List, learning_rate: float, **kwargs):
        super(ValueNetwork, self).__init__()

        input_dim = obs_dim[0]
        activation = kwargs.get('value_activation', VALUE_ACTIVATION)
        hidden_layers = kwargs.get('value_layer', VALUE_LAYER)
...
class MaskedPPO:
    def __init__(self, env, config: dict, logger: Logger = None):
        ...
        # create networks according to policy
        policy_parameter = config.get('policy')
        assert policy_parameter in AGENT_POLICY_DICT and policy_parameter in AGENT_VALUE_DICT, \
            'Selected policy has not been registered in ppo_masked.py'

        policy_net_class = AGENT_POLICY_DICT[config.get('policy')]
        value_net_class = AGENT_VALUE_DICT[config.get('policy')]

        self.policy_net = globals().get(policy_net_class)\
            (action_dim=env.action_space.n, obs_dim=env.observation_space.shape, num_tasks=env.num_tasks*env.num_jobs, **config)
        self.value_net = globals().get(value_net_class)(obs_dim=env.observation_space.shape, num_tasks=env.num_tasks*env.num_jobs, **config)
    ...
    def load(cls, file: str, config: dict, logger: Logger = None):
        ...
        :param config: Dictionary with parameters to specify PPO attributesf
        :param logger: Logger
