    ...
    def __init__(self, env, config: Dict, logger: Logger = None):
        ...
        if self.seed is not None:
            ...
            assert policy_parameter in AGENT_POLICY_DICT and policy_parameter in AGENT_VALUE_DICT, \
                'Selected policy has not been registered in ppo.py'
            ...
            self.policy_net = globals().get(policy_net_class) \
                (action_dim=env.action_space.n, obs_dim=env.observation_space.shape,
                 num_tasks=env.num_tasks * env.num_jobs, **config)
            self.value_net = globals().get(value_net_class)(obs_dim=env.observation_space.shape,
                                                            num_tasks=env.num_tasks * env.num_jobs, **config)
