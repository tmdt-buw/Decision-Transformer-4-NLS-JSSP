...
def final_evaluation(config: dict, data_test: List[List[Task]], logger: Logger, table: str = "default",
                     taillard_test: bool = False):
    """
    ...
    :param table: Name of the table logged in wandb
    :param taillard_test: Taillard instances are used for testing
    ...
    if taillard_test:
        evaluation_results = test.test_model_and_heuristic(config=config, model=best_model, data_test=data_test,
                                                           logger=logger, plot_ganttchart=False, log_episode=False,
                                                           taillard_test=True)
    else:
        evaluation_results = test.test_model_and_heuristic(config=config, model=best_model, data_test=data_test,
    ...
    logger.write_to_wandb_summary(evaluation_results, config=config, table=table)
...
def main(config_file_name: dict = None, external_config: dict = None) -> None:
    ...
    # function to determine instance number according to instance size
    if config['taillard_test']:
        data_taillard = DataHandler.load_taillard_instances(config)
    ...
    # evaluate results
    # Todo Merlin: Möglichkeit implementieren, dass Taillard Größe von Trainings Größe abweicht
    if config['taillard_test']:
        final_evaluation(config=config, data_test=test_data, logger=logger, table="no_solver")
        final_evaluation(config=config, data_test=data_taillard, logger=logger, table="taillard", taillard_test=True)
    else:
