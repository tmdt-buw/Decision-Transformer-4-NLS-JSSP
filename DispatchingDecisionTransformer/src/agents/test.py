...
def log_results(plot_logger: Logger, inter_test_idx: Union[int, None], heuristic: str,
                env, handler: EvaluationHandler, config: dict) -> None:
    """
    ...
    :param env: Environment object
    :param handler: EvaluationHandler object
    :param config: Training config used to determine if gantt chart should be plotted in wandb
    ...
    # create gantt chart if it's enabled in training cfg
    if config['wandb_gantt']:
        ...
        # Log rest of the table apart from gantt chart
        if heuristic:
            plot_logger.add_row_to_wandb_table(heuristic, **measures)
        else:
            if inter_test_idx is None:
                plot_logger.add_row_to_wandb_table("RL-Agent", **measures)
            else:
                plot_logger.add_row_to_wandb_table(f"RL-Agent {inter_test_idx}", **measures, gantt=False)
...
def test_model(env_config: Dict, data: List[List[Task]], logger: Logger, plot: bool = None, log_episode: bool = None,
    ...
    for test_i in range(len(data)):
        ...
        if log_episode:
            log_results(logger, intermediate_test_idx, heuristic_id, environment, evaluation_handler, env_config)
...
def test_model_and_heuristic(config: dict, model, data_test: List[List[Task]], logger: Logger,
                             plot_ganttchart: bool = False, log_episode: bool = False,
                             taillard_test: bool = False) -> dict:
    """
    ...
    :param log_episode: If true, calls the log function to log episode results as table
    :param taillard_test: If true, Taillard instances are used for benchmarking

    :return: Dict with evaluation_result dicts for the agent and all heuristics which were tested

    """
    if taillard_test:
        print('Benchmarking model using Taillard instances...')
    elif config['taillard_test']:
        print('Testing model and heuristics... ')
    else:
    ...
    # add lower and upper bound of taillard instance
    if taillard_test:
        res = get_taillard_bounds(config=config)
        results.update({'taillard': res})
        results = EvaluationHandler.add_lb_gap_to_results(results)

    # test solver and calculate optimality gap
    if not config['taillard_test']:
...
def get_taillard_bounds(config: dict = None) -> int:

    # Get Taillard number using slicing
    ta_file = config.get('taillard_file')
    index1 = ta_file.index("a")
    index2 = ta_file.index(".")
    taillard_num = ta_file[index1+1:index2]

    # read lower/upper bound from taillard_raw file

    with open(TAILLARD_RAW_DIRECTORY / f"ta{taillard_num}.txt") as handle:
        data = handle.read()
    data_arr = data.split()

    lower_bound = data_arr[19]
    upper_bound = data_arr[18]

    # add evaluation_results
    evaluation_results = {}
    evaluation_results['rew_mean'] = 0
    evaluation_results['rew_std'] = 0
    evaluation_results['rew_best'] = 0
    evaluation_results['rew_best_count'] = 0
    evaluation_results['rew_worst'] = 0
    evaluation_results['tardiness_mean'] = 0
    evaluation_results['tardiness_std'] = 0
    evaluation_results['tardiness_max_mean'] = 0
    evaluation_results['makespan_mean'] = 0
    evaluation_results['lower_bound'] = int(lower_bound)
    evaluation_results['upper_bound'] = int(upper_bound)
    evaluation_results['gap_to_lb'] = 0
    evaluation_results['rew_worst_quantile_border'] = 0
    evaluation_results['rew_cvar'] = 0
    evaluation_results['rew_perc_good_solutions'] = 0
    evaluation_results['num_tests'] = 0
