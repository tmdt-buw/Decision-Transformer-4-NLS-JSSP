...
class Env_wLS(gym.Env):
    """
    Environment for scheduling optimization with left shifting.
    ...
    def execute_action(self, job_id: int, task: Task, machine_id: int) -> None:
        ...
        if task.task_index == 0:
            end_time_of_preceding_task = 0
        else:
            preceding_task = self.tasks[self.task_job_mapping[(job_id, task.task_index - 1)]]
            end_time_of_preceding_task = preceding_task.finished

        # check earliest possible time to schedule according to preceding task and needed machine
        start_time = max(end_time_of_preceding_task, self.ends_of_machine_occupancies[machine_id])
        ...
        # check if a left shift is possible and adjust start_time
        slot = self.shift_left(task)
        # slot = None
        if slot is not None:
            start_time = slot[0]
        ...
        if end_time > self.ends_of_machine_occupancies[machine_id]:
        ...
        # if self.num_steps == 35:
        #     print('stop')

    def shift_left(self, task):
        """
        Shifts the task to the left if possible. Only works for the standard JSSP for now
        ...
        if task.task_index == 0:
            ...
            preceding_task_in_job = self.tasks[self.task_job_mapping[(task.job_index, task.task_index - 1)]]
            earliest_possible_start_time = preceding_task_in_job.finished

        # determine the free time slots on the required machine
        time_slots = [[0, earliest_possible_start_time]]
        suitable_slot = None
        for t in self.tasks:
            # check if the task is not the task itself and on the same machine
            if t != task and t.selected_machine == task.selected_machine and t.done:
                if t.finished > earliest_possible_start_time:
                    time_slots.append([t.started, t.finished])

        time_slots = np.sort(np.array(time_slots).flatten())

        # find slots between every finished and started tasks
        for i in range(int(len(time_slots)/2 - 1)):
            start_of_slot = time_slots[2*i+1]
            end_of_slot = time_slots[2*i+2]
            if end_of_slot - start_of_slot >= task.runtime and start_of_slot >= earliest_possible_start_time:
                suitable_slot = [start_of_slot, end_of_slot]
                break
