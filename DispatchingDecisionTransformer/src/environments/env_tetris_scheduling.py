...
class Env(gym.Env):
    ...
    def __init__(self, config: dict, data: List[List[Task]]):
        ...
        self.last_mask: numpy.ndarray = np.ones(self.num_jobs)
        ...
        # observation space
        self.observation_strategy = config.get('observation_strategy', ['basic'])
    ...
    def get_instance_lower_bound_makespan(self) -> int:
        machine_runtimes = numpy.arange(self.num_machines)
        for task in self.data[self.data_idx]:
            machine_for_task = task.machines.index(1)
            machine_runtimes[machine_for_task] += task.runtime
        return -(machine_runtimes.max())
    ...
    def state_obs(self) -> List[float]:
        ...
        """
        observation = np.concatenate([ObservationParser.parse(self, obs) for obs in self.observation_strategy])
    ...
    def get_previous_tasks(self):
        """returns the last tasks that have been scheduled"""
        previous_tasks = []
        for job in range(self.num_jobs):
            if self.job_task_state[job] == 0:  # means that its the first task of the job
                previous_tasks.append(0)
            else:
                task_position = self.task_job_mapping[(job, self.job_task_state[job] - 1)]
                previous_tasks.append(self.tasks[task_position])
