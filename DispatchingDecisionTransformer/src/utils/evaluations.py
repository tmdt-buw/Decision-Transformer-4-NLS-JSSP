...
class EvaluationHandler:
    ...
    def evaluate_test(self) -> dict:
        ...
        evaluation_results['lower_bound'] = np.min(self.makespan)
        evaluation_results['upper_bound'] = np.max(self.makespan)
    ...
    @classmethod
    def add_lb_gap_to_results(cls, results: dict) -> dict:
        """
        If taillard benchmarking is used, gap to lower bound (equal to solver solution) is added

        :param results: Dictionary with test results

        :return: Updated dictionary with test results now including lower bound gap

        """
        if 'taillard' in results:
            lower_bound = results['taillard']['lower_bound']
            for agent, result in results.items():
                if agent == 'taillard':
                    gap = 0
                else:
                    gap = result['makespan_mean'] - lower_bound
                results[agent].update({'gap_to_lb': gap})
        return results
