...
WANDB_PROJECT: str = 'observation_experiments'
WANDB_ENTITY: str = 'schlably-merlin'
...
WANDB_TAILLARD_TABLE_COLUMNS: List[str] = ['Agent', 'Mean Reward', 'Mean Makespan', 'Lower Bound', 'Upper Bound',
                                           'Gap to LB']
...
class Logger:
    ...
    def add_row_to_wandb_table(self, agent: str, gantt_chart: PIL.Image = None, **kwargs) -> None:
        """
        ...
        if self.wandb_run:
            ...
            if not self.wandb_table:
                if gantt_chart:
                    ...
                    table_columns_no_gantt = WANDB_TABLE_COLUMNS.copy()
                    table_columns_no_gantt.remove("Ganttchart")
                    self.wandb_table = wandb.Table(columns=table_columns_no_gantt)
    ...
    def write_to_wandb_summary(self, evaluation_results: dict, config: dict, table: str = "default"):
        """
        ...
        :param taillard_test: Bool that determines if taillard instances are used for benchmarking
        ...
        if self.wandb_run:
            if table == "default":
            ...
            elif table == "no_solver":
                columns_no_solver = WANDB_FINAL_EVALUATION_TABLE_COLUMNS.copy()
                columns_no_solver.remove('Gap To Solver')
                final_evaluation_table = wandb.Table(columns=columns_no_solver)
                # iterate overall all agent whose results are saved in evaluation_results
                for agent in evaluation_results.keys():
                    log_data = []
                    log_data.append(str(agent))
                ...
                taillard_table = wandb.Table(columns=WANDB_TAILLARD_TABLE_COLUMNS)
                # iterate overall all agent whose results are saved in evaluation_results
                for agent in evaluation_results.keys():
                    log_data = []
                    log_data.append(str(agent))
                    log_data.append(evaluation_results[agent]['rew_mean'])
                    log_data.append(evaluation_results[agent]['makespan_mean'])
                    log_data.append(evaluation_results[agent]['lower_bound'])
                    log_data.append(evaluation_results[agent]['upper_bound'])
                    log_data.append(evaluation_results[agent]['gap_to_lb'])
                    taillard_table.add_data(*log_data)
                self.wandb_run.log({'Taillard Table': taillard_table})

            else:
                raise TypeError("Unknown type of evaluation table")