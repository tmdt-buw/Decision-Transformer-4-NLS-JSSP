...
TAILLARD_INSTANCE_DIRECTORY: Path = Path(__file__).parent.parent.parent.parent\
                                    / 'data' / 'benchmark_instances' / 'taillard'
...
class DataHandler:
    ...
    @classmethod
    def load_taillard_instances(cls, config: dict):
        """
        Loads the taillard instances used for benchmarking
        :param config: Config with at least TAILLARD_FILE
        :return: Requested Taillard instance

        """
        taillard_instance_file = TAILLARD_INSTANCE_DIRECTORY / config.get('taillard_file')
        taillard_data = None
        if taillard_instance_file.exists():
            with open(taillard_instance_file, 'rb') as handle:
                taillard_data = pickle.load(handle)
        else:
            raise TypeError("Activated Taillard benchmarking but no correct Taillard instance path was given.")

        return taillard_data
